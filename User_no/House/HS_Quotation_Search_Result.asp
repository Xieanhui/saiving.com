...
Function Get_While_Info(Add_Sql)
	...
	Hs_Rs.AbsolutePage=cPageNo
	  ...
	  FOR int_Start=1 TO int_RPP 
		...
		Get_Html = Get_Html & "<td align=""center"" onmouseover=""this.style.cursor = 'hand'"" onclick=""javascript:if(TD_U_"&Hs_Rs("ID")&".style.display=='') TD_U_"&Hs_Rs("ID")&".style.display='none'; else TD_U_"&Hs_Rs("ID")&".style.display='';"" title='点击查看更多信息'>"&Hs_Rs("ID")&"</td>" & vbcrlf
		...
		elseif Hs_Rs("UserNumber")<>"" then 
			...
			if Str_Tmp<>"" then 
				Str_Tmp = "<span  onmouseover=""this.style.cursor = 'hand'"" id="""&Hs_Rs("UserNumber")&""" title="""&Str_Tmp&""">房产公司:"&Hs_Rs("UserNumber")&"</span>"
			else
				Str_Tmp = "未知的用户"
			end if
		else
		...
		Get_Html = Get_Html & "<tr class=""hback"">" & vbcrlf &"<td>楼盘位置:"&Hs_Rs("Position") & "</td><td>楼盘方位:"&Hs_Rs("Direction")&"</td><td>项目类别:"&Hs_Rs("Class") & "</td><td>开盘日期:"&Hs_Rs("OpenDate")& "</td></tr>" & vbcrlf
		Get_Html = Get_Html & "<tr class=""hback"">" & vbcrlf &"<td>预售许可证:"&Hs_Rs("PreSaleNumber") & "</td><td>发证日期:"&Hs_Rs("IssueDate")&"</td><td>本次预售范围:"&Hs_Rs("PreSaleRange") & "</td><td>房屋状况:"&Hs_Rs("Status")& "</td></tr>" & vbcrlf
		Get_Html = Get_Html & "<tr class=""hback"">" & vbcrlf &"<td>报价:"&Hs_Rs("Price") & "</td><td>发布日期:"&Hs_Rs("PubDate")&"</td><td>联系电话:"&Hs_Rs("Tel") & "</td><td>点击:"&Hs_Rs("Click")& "</td></tr>" & vbcrlf
...
<table width="98%" border="0" align="center" cellpadding="3" cellspacing="1" class="table">
  <form name="form1" method="post" action="?Act=Del&TableName=<%=request.QueryString("TableName")%>">
    <tr  class="hback"> 
      ...
      <td align="center" class="xingmu"><a href="javascript:OrderByName('HouseName')" class="sd"><b>房源名称</b></a> 
