...
Function Get_While_Info(Add_Sql)
	...
	Str_Tmp = "TID,HouseStyle,PubDate,Audited,UseFor,Class,Position,CityArea,Price,HouseStyle,Area,Floor,BuildDate,equip,Decoration,LinkMan,Contact,Period,Remark"
	This_Fun_Sql = "select "&Str_Tmp&" from FS_HS_Tenancy"
	Arr_Tmp = split(Str_Tmp,",")
	for each Str_Tmp in Arr_Tmp
		Req_Str = NoSqlHack(Trim(request(Str_Tmp)))
		if Req_Str<>"" then 				
			select case Str_Tmp
			case "TID","UseFor","Class","Price","Area","BuildDate","PubDate","Audited","PicNumber","isRecyle"
			''数字,日期
				regxp = "|<|>|=|<=|>=|<>|"
	...
	Hs_Rs.AbsolutePage=cPageNo
	  ...
	  FOR int_Start=1 TO int_RPP 
		...
		Get_Html = Get_Html & "<td align=""center"" onmouseover=""this.style.cursor = 'hand'"" onclick=""javascript:if(TD_U_"&Hs_Rs("TID")&".style.display=='') TD_U_"&Hs_Rs("TID")&".style.display='none'; else TD_U_"&Hs_Rs("TID")&".style.display='';"" title='点击查看更多信息'>"&Hs_Rs("TID")&"</td>" & vbcrlf
		...
		Get_Html = Get_Html & "<td align=""center"">"&Replacestr(Hs_Rs("Class"),"1:出租,2:求租,3:出售,4:求购,5:合租,6:转让")&"</td>" & vbcrlf
		...
		''++++++++++++++++++++++++++++++++++++++点开时显示详细信息。
		Get_Html = Get_Html & "<tr class=""hback"" id=""TD_U_"& Hs_Rs("TID") &""" style=""display:none""><td colspan=20>" & vbcrlf
		Get_Html = Get_Html & "<table width=""100%"" height=""30"" border=""0"" cellspacing=""1"" cellpadding=""2"" class=""table"">" & vbcrlf 
		Get_Html = Get_Html & "<tr class=""hback"">" & vbcrlf &"<td>用途:"&Replacestr(Hs_Rs("UseFor"),"1:住房,2:写字间") & "</td><td>房源地址:"&Hs_Rs("Position")&"</td><td>区县:"&Hs_Rs("CityArea") & "</td><td>金额:"&Hs_Rs("Price")& "</td></tr>" & vbcrlf
		Str_Tmp = Hs_Rs("Floor")
		Str_Tmp = "总层" & left(Str_Tmp,instr(Str_Tmp,",") - 1)&",第"& mid(Str_Tmp,instr(Str_Tmp,",") + 1,len(Str_Tmp)) &"层"
		Get_Html = Get_Html & "<tr class=""hback"">" & vbcrlf &"<td>面积:"&Hs_Rs("Area") & "</td><td>楼层:"&Str_Tmp&"</td><td>建筑年代:"&Hs_Rs("BuildDate")
		Str_Tmp = Hs_Rs("equip")
		...
		Get_Html = Get_Html & "</td><td>配套设施:"&Str_Tmp& "</td></tr>" & vbcrlf
		Get_Html = Get_Html & "<tr class=""hback"">" & vbcrlf &"<td>装修情况:"&Replacestr(Hs_Rs("Decoration"),"1:简单装修,2:中档装修,3:高档装修") & "</td><td>联系人:"&Hs_Rs("LinkMan")&"</td><td>联系方式:"&Hs_Rs("Contact") & "</td><td>有效期:"&Hs_Rs("Period")& "</td></tr>" & vbcrlf
		Get_Html = Get_Html & "<tr class=""hback"">" & vbcrlf &"<td colspan=10>备注:"&Hs_Rs("Remark")&"</td></tr>" & vbcrlf
...
<table width="98%" border="0" align="center" cellpadding="3" cellspacing="1" class="table">
  <form name="form1" method="post" action="?Act=Del&TableName=<%=request.QueryString("TableName")%>">
    <tr  class="hback"> 
      <td align="center" class="xingmu" ><a href="javascript:OrderByName('TID')" class="sd"><b>〖ID〗</b></a> 
      ...
      <td align="center" class="xingmu"><a href="javascript:OrderByName('Class')" class="sd"><b>类型</b></a> 
        <span id="Show_Oder_Class"></span></td>
