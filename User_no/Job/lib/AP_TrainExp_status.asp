...
if session("FS_UserNumber")<>"" then
	...
	if not resumeRs.eof then 
		...
		Response.Write("<td class='xingmu' align='center'>培训机构</td>"&vbcrlf)
		Response.Write("<td class='xingmu' align='center'>证书</td>"&vbcrlf)
		...
		while not resumeRs.eof
			...
			Response.Write("<td class='hback' align='center'><a href='#' onClick=""getResumeForm('resume_container','trainexp','"&resumeRs("bid")&"','edit')"">修改/查看</a>&nbsp;&nbsp;|&nbsp;&nbsp;<a href='#' onClick=""Delete('trainexp','"&resumeRs("bid")&"')"">删除</a></td>"&vbcrlf)
			Response.Write("</tr>"&vbcrlf)	
			resumeRs.movenext
		wend
		...
		Else
			Response.Write("<td>你还没有填写培训经历！现在创建你的培训经历</td>"&vbcrlf)
		End if
		Response.Write("</tr>"&vbcrlf)
		Response.Write("</table>"&vbcrlf)
	End if
