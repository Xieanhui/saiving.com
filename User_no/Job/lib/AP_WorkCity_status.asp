...
if session("FS_UserNumber")<>"" then
	...
	if not resumeRs.eof then 
			...
			Response.Write("<td class='xingmu'>省/市</td>"&vbcrlf)
			...
			while not resumeRs.eof
				...
				Response.Write("<td class='hback'align='center' width='20%'><a href='#' onClick=""Delete('workcity','"&resumeRs("bid")&"')"">删除</a></td>"&vbcrlf)
				Response.Write("</tr>"&vbcrlf)	
				resumeRs.movenext
			wend
		...
		Else
			Response.Write("<td>你还没有选择工作地点！现在设置</td>"&vbcrlf)
		End if
		Response.Write("</tr>"&vbcrlf)
		Response.Write("</table>"&vbcrlf)
	End if
