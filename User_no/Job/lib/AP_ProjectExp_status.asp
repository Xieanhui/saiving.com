...
if session("FS_UserNumber")<>"" then
	Set resumeRs=Conn.execute("select bid,BeginDate,EndDate,Project,SoftSettings,HardSettings,Tools,ProjectDescript,Duty from FS_AP_Resume_ProjectExp where UserNumber='"&session("FS_UserNumber")&"'")
	if not resumeRs.eof then 
			...
			Response.Write("<td class='xingmu' align='center'>项目名称</td>"&vbcrlf)
			...
			while not resumeRs.eof
				...
				Response.Write("<td class='hback' align='center'>"&resumeRs("Project")&"</td>"&vbcrlf)
				Response.Write("<td class='hback' align='center'><a href='#' onClick=""getResumeForm('resume_container','projectexp','"&resumeRs("bid")&"','edit')"">修改/查看</a>&nbsp;&nbsp;|&nbsp;&nbsp;<a href='#' onClick=""Delete('projectexp','"&resumeRs("bid")&"')"">删除</a></td>"&vbcrlf)
				Response.Write("</tr>"&vbcrlf)	
				resumeRs.movenext
			wend
		...
		Else
			Response.Write("<td>你还没有填写项目经历！现在创建你的项目经历</td>"&vbcrlf)
		End if
		Response.Write("</tr>"&vbcrlf)
		Response.Write("</table>"&vbcrlf)
	End if
