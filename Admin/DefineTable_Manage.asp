...
Sub DelClass()       
	...
	''+++++++++++++++++
	response.Redirect("Success.asp?ErrorUrl=DefineTable_Manage.asp&ErrCodes=<li>恭喜，删除成功。</li>")
End Sub
...
Sub SaveClass()
	...
	''新增
		if CheckCardCF(NoSqlHack(Trim(request.Form("DefineName"))),request.Form("ParID"))<>"" then 
			response.Redirect("error.asp?ErrCodes=<li>同级别的类别: "&request.Form("DefineName")&" 已经存在。</li>")
			response.End()
		end if	
	...
	''修改
		...
		''**********************
		''当自定义字段类别要变更时判断当前级别加上其子级再加上要变更的类别一共是否在4级以内。若不再这返回。
		...
		response.Redirect("error.asp?ErrCodes=<li>必要的自定义字段ID没有提供。</li>")	
		response.End()
	end if
	...
	if NoSqlHack(request.Form("ParID"))<>"" then 
		...
		VClass_Rs.update
		VClass_Rs.close
		response.Redirect("Success.asp?ErrorUrl="&server.URLEncode( "DefineTable_Manage.asp?Act=Add&DefineID="&NoSqlHack(request.Form("ParID"))&"&VCText="&NoSqlHack(request.Form("VCText")) )&"&ErrCodes=<li>保存成功。</li>")
	end if
	'''修改
	if NoSqlHack(request.Form("DefineID"))<>"" then 
		...
		if Edit_PartID<>"" then 
			...
			''取得变更后的父级ID的父级ID，并传给返回的DefineID，以便显示父级ID同级别的所有类别
		...
		VClass_Rs.close
		response.Redirect("Success.asp?ErrorUrl="&server.URLEncode( "DefineTable_Manage.asp?Act=View&DefineID="&Edit_PartID )&"&ErrCodes=<li>保存成功。</li>")
	end if
End Sub

Function CheckCardCF(DefineName,ParentID)
''检查录入的是否重复,重复则返回DefineName,不重复则返回""
	Dim CheckCardCF_Rs
...
case "AddMode"
	...
	if str_Session <> "" then 
		...
		''按扭
			response.Write(" disabled title=""自定义字段分类最多一级!当前等级:第"&cstr(ubound(Arr_Tmp) + 1)&"级."" ")	
		end if
	end if
	''============================
case "SaveMode"
	...
	if str_Session <> "" then 
		...
		Arr_Tmp = split(str_Session,",")
		if ubound(Arr_Tmp)>0 then 
			response.Redirect("error.asp?ErrCodes=<li>自定义字段类别不能超过一级。</li>")	
			response.End()
		else
			response.Write("自定义字段分类最多一级!当前等级:第"&cstr(ubound(Arr_Tmp) + 1)&"级.")	
		end if
	end if
	''============================
case else
...
''递归调用显示分类
	...
	VClass_Rs.AbsolutePage=cPageNo
	  ...
	  FOR int_Start=1 TO int_RPP 
		...
		Get_Html = Get_Html & "<td align=""center""><a href=""DefineTable_Info_Manage.asp?Act=View&Add_Sql="& server.URLEncode(Encrypt( "ClassID="&VClass_Rs("DefineID") )) &"""  class=""otherset"">查看该类字段</FONT></a></td>" & vbcrlf
''		Get_Html = Get_Html & "<td align=""center""><a href=DefineTable_Manage.asp?Act=View&DefineID="&VClass_Rs("DefineID")&">"& VClass_Rs("DefineName") & "</a></td>" & vbcrlf
		Get_Html = Get_Html & "<td align=""center""><a href=""DefineTable_Info_Manage.asp?Act=Add&ClassID="&VClass_Rs("DefineID")&"""  class=""otherset"">新增该类字段</FONT></a></td>" & vbcrlf
''		Get_Html = Get_Html & "<td align=""center""><a href=""DefineTable_Manage.asp?Act=Add&DefineID="&VClass_Rs("DefineID")&"&VCText="&VClass_Rs("DefineName")&"""  class=""otherset"">新增</FONT></a></td>" & vbcrlf
		Get_Html = Get_Html & "<td align=""center""><a href=""DefineTable_Manage.asp?Act=Edit&DefineID="&VClass_Rs("DefineID")&"&VCText="&VClass_Rs("DefineName")&"""  class=""otherset"">设置</FONT></a></td>" & vbcrlf
	...
	Get_Html = Get_Html & "</td><td align=right><input type=""button"" name=""button1112"" value="" 新增同级栏目 "" onClick=""location='DefineTable_Manage.asp?Act=Add"& str_Url_Add &"'"">" & vbcrlf
...
''循环调用，通过传入的DefineID得到子级的DefineID，并且组合起来，一起传给DelClass过程以删除所有相关类别。
...
''层层导航作用 session("TopMenu_DaoHang_DefineID_List") = ',1,2,3,4,'
Dim str_Req_DefineID,str_Session,Sql_Session,Per_ID_Session,Str_Tmp,Str_Remove,Arr_Tmp,This_Fun_Sql
	...
	'response.Write("原"&session("TopMenu_DaoHang_DefineID_List"))
	',1,2,3,4,' 与 ',3,'
	...
	''截取
		...
		session("TopMenu_DaoHang_DefineID_List") = str_Session
		'response.Write("减"&session("TopMenu_DaoHang_DefineID_List"))
	else
	''添加
		str_Session = str_Session & NoSqlHack(request.QueryString("DefineID")) &","
		session("TopMenu_DaoHang_DefineID_List") = str_Session
		'response.Write("加"&session("TopMenu_DaoHang_DefineID_List"))
	end if
	end if
	''**************
...
if IsOk=false then 
	str_Url_Add = "&DefineID=0&VCText=无"	
else	
...
<table width="98%" border="0" align="center" cellpadding="5" cellspacing="1" class="table">
  <tr class="xingmu">
    <td class="xingmu"><a href="#" onMouseOver="this.T_BGCOLOR='#404040';this.T_FONTCOLOR='#FFFFFF';return escape('<div align=\'center\'>FoosunCMS5.0<br>  <BR>Copyright (c) 2006 Foosun Inc</div>')" class="sd"><strong>自定义管理</strong></a></td>
  </tr>
  <tr class="hback">
    <td class="hback"><!--<a href="DefineTable_Info_Manage.asp?Act=View">管理首页</a> -->
      <!-- | <a href="DefineTable_Manage.asp?Act=View&DefineID=<%=Get_PatID_TO_View(request.QueryString("DefineID"))%>" class="sd"><b>返回父级</b></a>
	   | <a href="#" onClick="javascript:history.back();" class="sd"><b>返回</b></a>-->
     <!-- | --><a href="DefineTable_Info_Manage.asp">字段内容管理</a></td>
  </tr>
</table>
  ...
  <form name="VCForm" id="VCForm" method="post" action="?Act=Del">
    <tr  class="hback"> 
      <td align="left" class="xingmu" colspan="5"> 
        <!-- | 分类导航：<a href="DefineTable_Manage.asp" class="sd"><b>首页</b></a>  >> <%=TopMenu_DaoHang()%>-->
      </td>
    </tr>
    <tr  class="hback"> 
      <td align="center"  class="xingmu">分类名称</td>
      <td width="25%" align="center" class="xingmu">查看自定义字段</td>
      <td width="25%" align="center" class="xingmu">新增自定义字段</td>
...
<table width="98%" border="0" align="center" cellpadding="5" cellspacing="1" class="table">
  <tr class="xingmu"> 
    <td class="xingmu">自定义管理</td>
  </tr>
  <tr class="hback"> 
    <td class="hback"><a href="DefineTable_Manage.asp?Act=View">管理首页</a> 
      <!-- | <a href="DefineTable_Manage.asp?Act=View&DefineID=<%=Get_PatID_TO_View(request.QueryString("DefineID"))%>" class="sd"><b>返回父级</b></a>
	   | <a href="#" onClick="javascript:history.back();" class="sd"><b>返回</b></a>-->
      | <a href="DefineTable_Info_Manage.asp">字段内容管理</a></td>
  </tr>
...
<table width="98%" border="0" align="center" cellpadding="3" cellspacing="1" class="table">
  <form name="VCForm" id="VCForm" method="post" action="?Act=Save">
    <tr  class="hback"> 
      <td colspan="2" align="left" class="xingmu" >新增自定义字段分类信息 自定义字段分类目前一级</td>
    </tr>
    <tr  class="hback"> 
      <td align="right">上一级自定义字段名称</td>
    ...
    <tr class="hback"> 
      <td align="right">新的自定义字段名称：</td>
      <td width="596"><input name="DefineName" type="text" id="DefineName" size="50" onBlur="if(this.value==''){Chk_vCName.innerText='该项必须填写';this.focus();} else if(Chk_vCName.innerText!='')Chk_vCName.innerText=''" />
        <span id="Chk_vCName" class="tx"></span> </td>
    </tr>
    <tr  class="hback"> 
      <td colspan="4"> <table border="0" width="100%" cellpadding="0" cellspacing="0">
          <tr> 
            <td align="center"> <input type="submit" <%call Say_Limit_Class("AddMode")%> name="SaveClass" id="SaveClass" value=" 保存 " onClick="if(DefineName.value==''){alert('必要的参数必须填写');DefineName.focus();return false}" /> 
              &nbsp; <input type="reset" name="ReSet" id="ReSet" value=" 重置 " /> 
            </td>
            <td width="50" align="right"> <a href="DefineTable_Manage.asp?Act=View&DefineID=<%=Get_PatID_TO_View(View_Par_ID)%>">返回父级</a> 
            </td>
            <td width="50" align="right"> <a href="#" onClick="javascript:history.back();">返回</a> 
            </td>
          </tr>
        </table></td>
    </tr>
...
<table width="98%" border="0" align="center" cellpadding="3" cellspacing="1" class="table">
  <form name="VCForm" id="VCForm" method="post" action="?Act=Save">
    <tr  class="hback"> 
      <td colspan="2" align="left" class="xingmu" >修改自定义字段分类信息 自定义字段分类目前一级</td>
    </tr>
    <tr  class="hback" style="display:none"> 
      <td align="right">变更上级类别<br>
        <span class="tx">若不变更请重置</span></td>
...
<!---联动菜单结束--- > -->
    ...
    <tr  class="hback"> 
      <td align="right">自定义字段名称</td>
      <td width="596"> <input name="DefineName" id="DefineName" type="text" size="50" value="<%=request.QueryString("VCText")%>" onBlur="if(this.value==''){Chk_vCName.innerText='该项必须填写';this.focus()} else if(Chk_vCName.innerText!='')Chk_vCName.innerText=''" />
    ...
    <tr  class="hback"> 
      <td colspan="4"> <table border="0" width="100%" cellpadding="0" cellspacing="0">
          <tr> 
            <td align="center"> <input type="submit" <%call Say_Limit_Class("AddMode")%> name="SaveClass2" id="SaveClass2" value=" 保存 " onClick="if(DefineName.value==''){DefineName.focus();return false}" /> 
              &nbsp; <input type="reset" name="ReSet2" id="ReSet2" value=" 重置 " /> 
            </td>
            <td width="50" align="right"> <a href="DefineTable_Manage.asp?Act=View&DefineID=<%=Get_PatID_TO_View(request.QueryString("DefineID"))%>">返回父级</a> 
            </td>
            <td width="50" align="right"> <a href="#" onClick="javascript:history.back();">返回</a> 
            </td>
          </tr>
