   ...
   Public Function chkinputchar(f_char)
	...
	Public Function GetTodayNewsCount(f_classID) 
			...
			If G_IS_SQL_DB=0 Then
				f_obj_cnews_rs.Open "Select ID from FS_NS_News where ClassID='"& f_classID &"' and datevalue(addtime)=#"&date()&"#",Conn,1,1
			Else
				f_obj_cnews_rs.Open "Select ID from FS_NS_News where ClassID='"& f_classID &"' and convert(varchar(10),addTime,120)='"&date()&"'",Conn,1,1
			End If
	...
	Public Function strFileNameRule(str,f_idTF,f_id)
		strFileNameRule = ""
		Dim f_strFileNamearr,f_str0,f_str1,f_str2,f_str3,f_str4,Getstr,f_str5
	...
	Public Function GetKeywordslist(f_char,f_number)
		...
		f_obj_kw_Rs.Open "Select top 5 GID,G_Name,G_Type,isLock from FS_NS_General where G_Type ="& f_number &" and isLock=0  order by GID desc",Conn,1,1
	...
	Public Function GetCustClassID(f_custclassid)
		Dim obj_cust_rs
		set obj_cust_rs = Conn.execute("select DefineID from FS_NS_NewsClass where Classid='"& f_custclassid &"'")
	...
	Public Function GetUserName(f_strNumber)
		Dim RsGetUserName
		Set RsGetUserName = User_Conn.Execute("Select UserName From FS_ME_Users Where UserNumber = '"& f_strNumber &"'")
