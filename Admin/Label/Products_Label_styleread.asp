...
<%
	...
	Dim Conn,obj_label_Rs,SQL,regEx,result
	MF_Default_Conn
	'session判断
	MF_Session_TF 
	If Request("id")="" Then
  ...
  <tr > 
	...
	<table width="100%" border="0" align="center" cellpadding="3" cellspacing="1" class="table">
        <%
			...
			SQL = "Select  StyleName,Content,AddDate from FS_MF_Labestyle where id=" & Request.QueryString("id")
			...
			if not obj_label_Rs.eof then
				...
				regEx.Global = True '  
			    result = regEx.replace(obj_label_Rs("Content"),"<img src='../images/default.png' border='0'/>") 
			%>
