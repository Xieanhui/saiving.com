...
<td width="8%" class="hback" align="center"><% = obj_Label_Rs("ID") %></td>
    ...
    <td width="21%"><a href="All_Label_style.asp?id=<%=obj_Label_Rs("ID")%>&Label_Sub=<%=obj_Label_Rs("StyleType")%>&DelTF=1" onClick="{if(confirm('确定删除此引用样式吗？')){return true;}return false;}">删除</a> <a href="All_Label_style_txt.asp?Action=Add&type=edit&id=<%= obj_Label_Rs("id")%>&Label_Sub=<%= obj_Label_Rs("StyleType")%>&ClassID=<% If Request.QueryString("ClassId")<>"" And IsNumeric(Request.QueryString("ClassId")) Then : Response.Write Request.QueryString("ClassId") : Else : Response.Write "0" : End IF %>"  target="_self">TXT</a></td>
  </tr>
...
{
	//Label_Form.TxtFileds.value = Label_Form.TxtFileds.value+InsertValue;//修改文本添加标签 by sicend
	try
	    {
...
<select name="NewsFields" onChange="Insertlabel_Sel(this)">
  ...
  <option value="<a href={NS:FS_NewsURL}>{NS:FS_NewsTitle}</a>">新闻标题(截断)，不可使用于title中</option>
  <option value="{NS:FS_NewsTitleAll}">新闻完整标题(不带New标志)</option>
