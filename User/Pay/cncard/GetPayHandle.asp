...
<%
Dim c_mid,c_order,c_orderamount,c_ymd,c_transnum,c_succmark,c_moneytype,c_cause,c_memo1,c_memo2,c_signstr,srcStr,r_signstr
...
c_signstr		= NoSqlHack(request("c_signstr"))		'云网支付网关对已上信息进行MD5加密后的字符串
	...
	rs_isp.open "select c_isp,c_user,c_pass,c_url,c_gurl From FS_ME_Pay where c_isp=1",User_Conn,1,3
	if rs_isp.eof then
	...
	<table width="98%" height="135" border="0" align="center" cellpadding="1" cellspacing="1" class="table">
		...
		IF c_succmark="Y" THEN
			...
			if CSTR(RsPay("IsPay"))="1" then
				...
				rs_u.open "select Content,M_PayDate,OrderNumber,M_state,isPay,isLock,IsSuccess From FS_ME_Order where OrderNumber='"& c_order &"' and UserNumber='"&Fs_User.UserNumber&"'",User_Conn,1,3
				rs_u("M_PayDate")=now
				rs_u("M_state")=1
				rs_u("isLock")=0
				rs_u("IsSuccess")=1
